...
function corporateAddressAnimDesk(){
    ...
    TweenMax.staggerTo('.cityAdd .pin', 0.3, {opacity:1, scale:1, ease:Sine.easeInOut, onComplete:function(){
        TweenMax.set('.add_ahmedabad', {display:'block'});
        TweenMax.to('.add_ahmedabad', 0.5, {opacity:1, x:0, ease:Sine.easeOut});
        //TweenMax.to('.ahmedabad_loc .line', 0.5, {width:'100%', ease:Sine.easeOut});
        TweenMax.to('.ahmedabad_loc .pin', 0.3, {opacity:1, top:'0', ease:Sine.easeOut},'pin');
        TweenMax.to('.ahmedabad_loc .base', 0.3, {opacity:1, scale:1, ease:Sine.easeOut},'pin');
        TweenMax.to('.ahmedabad_loc .city_name', 0.3, {opacity:'1', display:'block', ease:Sine.easeOut},'pin');
		//$('.ahmedabad_loc .pin').attr("src","assets/images/pointer-contact-corporate.png");
    }}, 0.08);
    ...
    $('.cityAdd .pointers').each(function(){
        $(this).bind('click', function(){
			...
			//$(".cityAdd .pin").attr("src","assets/images/pointer-contact.png");;
			...
			//$(this).find('.pin').attr("src","assets/images/pointer-contact-corporate.png");
...
function setAddressOnMob(){
    var winWT = $(window).innerWidth();
    if(winWT <= 1023){
        var addArr = $('.address_det');
        console.log(addArr.length);
        $(addArr[0]).css({'display':'block'});

        $('.citiesDropCon .citiesSelectBox').on('change', function(){
            var currValue = $(this).children("option:selected").index();
            $(addArr).css({'display':'none'});
            $(addArr[currValue]).fadeIn(100);
            /*if(currValue == 0){
                $(addArr).css({'display':'none'});
                $(addArr[0]).fadeIn(300);
            }
