...
if($page==1)
{
    ...
    <div class="row mt-3">
        <div class="col col-xsm-12">
            <h1 class="fs-3">Pending Transactions</h1>
        </div>
    ...
    // for total coupons 
    $sql="select * from transactions where status='pending' order by timestamp desc limit {$offset}, {$limit}";
    ...
    if(mysqli_num_rows($result)>0)
    {
        ...
        while($row=mysqli_fetch_assoc($result))
        {
            ...
            $output .='
                ...
                <td>'.strtoupper(date('d-m-Y', strtotime($row['timestamp']))).'</td>
                <td class="d-flex justify-content-between">'.ucwords($row['status']).' <button class="btn btn-success btn-sm update-transaction-btn" data-transaction-id="'.$row['transaction_id'].'" data-bs-toggle="modal" data-bs-target="#updateTransactionModal">Update</button></td>
                ';
        ...
        while($row=mysqli_fetch_assoc($result))
        {
            ...
            $output .='
                ...
                <td class="d-flex justify-content-between">'.ucwords($row['status']).' 
                    <button class="btn btn-success btn-sm update-transaction-btn" data-transaction-id="'.$row['transaction_id'].'" data-bs-toggle="modal" data-bs-target="#updateTransactionModal">Update</button></td>
                ';
